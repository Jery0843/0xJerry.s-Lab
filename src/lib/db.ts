...
export class HTBMachinesDB {
  ...
  async createMachine(machine: any): Promise<any | null> {
    ...
    try {
      const prepared = await this.db.prepare(`
        ...
        (id, name, os, difficulty, status, is_active, summary, date_completed, tags, writeup, created_at, updated_at)
  ...
  async updateMachine(id: string, machine: any): Promise<any | null> {
    ...
    try {
      const prepared = await this.db.prepare(`
        UPDATE htb_machines 
        SET name = ?, os = ?, difficulty = ?, status = ?, is_active = ?, summary = ?, date_completed = ?, 
